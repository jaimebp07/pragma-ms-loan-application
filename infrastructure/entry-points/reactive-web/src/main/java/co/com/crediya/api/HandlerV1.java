...
public class HandlerV1 {
        ...
        public Mono<ServerResponse> updateLoanAplicationStatus(ServerRequest serverRequest) {
                return serverRequest.bodyToMono(UpdateStatusRqDTO.class)
                        .flatMap(dto ->
                        updateLoanApplicationStatusUseCase
                                .updateLoanApplicationStatus(dto.loanId(), dto.status(), Optional.ofNullable(dto.comment()))
                                .flatMap(updated -> ServerResponse.ok().bodyValue(updated))
                        )
                        .doOnError(e -> log.error("Error updating status", e))
                        .onErrorResume(e ->
                        ServerResponse.badRequest().bodyValue(new ErrorResponse("ERROR", e.getMessage()))
                        );
        }
